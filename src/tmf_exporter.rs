use crate::{TMFMesh, TMFPrecisionInfo,TMFExportError,TMF_MAJOR,MIN_TMF_MAJOR,TMF_MINOR,MIN_TMF_MINOR};
...
async fn write_mesh<W: std::io::Write>(mesh:&TMFMesh,name:&str,target:&mut W,p_info:&TMFPrecisionInfo)-> Result<(),TMFExportError>{
...
pub(crate) fn write_tmf_header<W: std::io::Write>(w: &mut W, mesh_count: u32) -> Result<(),TMFExportError> {
    w.write_all(b"TMF")?;
