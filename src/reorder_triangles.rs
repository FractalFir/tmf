use crate::IndexType;
fn reorder_triangles<const ARRAY_COUNT:usize>(triangles:&[&[IndexType]])->Box<[Box<[IndexType]>]>{
    ...
    for index in 0..(len/3){
        let max = max_deltas[index*3].max(max_deltas[index*3+1]).max(max_deltas[index*3+2]);
        //tri_deltas.push(max);
    ...
    for array_index in 0..ARRAY_COUNT{
        let mut new_index_array:Vec<_> = res_sorted.iter().map(|arrays|{arrays[array_index]}).collect();
...
fn read_susan_obj() {
    ...
    let (vertex_indices,normal_indices,uv_indices) = (indices[0].clone(),indices[1].clone(),indices[2].clone());
    assert_eq!(vertex_indices.len(),tmf_mesh.get_vertex_triangles().unwrap().len());
