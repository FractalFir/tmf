use crate::unaligned_rw::{UnalignedRWMode,UnalignedWriter,UnalignedReader};
use crate::IndexType;
...
fn find_longest_match(data:&[u64],pos:usize,scrollback_limit:usize)->(usize,usize){
    ...
    let mut best_len = 0;
    let start = if pos < scrollback_limit{0} else{pos - scrollback_limit};
...
fn save_compressed_array<T,Mapper:Fn(&T)->u64,W:std::io::Write>(data:&[T],mapper:Mapper,prec:UnalignedRWMode,target:&mut W)->std::io::Result<()>{
    println!("data.len():{}",data.len());
    let data:Box<_> = data.iter().map(|val| {mapper(val)}).collect();
...
fn read_compressed_array<T,Mapper:Fn(u64)->T,R:std::io::Read>(mapper:Mapper,prec:UnalignedRWMode,length:usize,src:&mut R)->std::io::Result<Box<[T]>>{
    ...
    let output:Box<_> = output.into_iter().map(|enc|{mapper(enc)}).collect(); 
    Ok(output)
}
pub fn save_triangles_lzz<W: std::io::Write>(
    ...
    save_compressed_array(triangles,|index|{(index - min) as u64},precision,writer)?;
    //writer.flush()
    Ok(())
}
#[test]
fn encode_ascii_lzz(){
    ...
    let mut out = Vec::new();
    save_compressed_array(&data[..],|input:&u8|{*input as u64},UnalignedRWMode::precision_bits(7),&mut out).unwrap();
    let data = read_compressed_array(|input:u64|{input as u8},UnalignedRWMode::precision_bits(7),data.len(),&mut &out[..]).unwrap();
