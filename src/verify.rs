...
impl TMFIntegrityStatus {
    ...
    pub fn is_err(&self)->bool{!matches!(self, Self::Ok)}
}
impl std::fmt::Display for TMFIntegrityStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::IndexOutsideVertexArray(index,length) => write!(f, "IndexOutsideVertexArray{{index:{index},length{length}}}"),
            Self::NormalArrayMissing => write!(f, "NormalArrayMissing"),
            Self::NormalsNotNormalized => write!(f, "NormalsNotNormalized"),
            Self::IndexOutsideNormalArray(index,length) => write!(f, "IndexOutsideNormalArray{{index:{index},length{length}}}"),
            Self::IndexOutsideUVArray(index,length) => write!(f, "IndexOutsideUVArray{{index:{index},length{length}}}"),
...
fn verify_vertices(mesh:&TMFMesh)->TMFIntegrityStatus{
    match mesh.get_vertices(){
        None=>if mesh.get_vertex_faces().is_some(){
            TMFIntegrityStatus::VertexArrayMissing
        }else{TMFIntegrityStatus::Ok},
        Some(vertices)=>{
            match mesh.get_vertex_faces(){
                Some(faces)=>{
                    for index in faces{
                        if *index > vertices.len() as IndexType{
                            return TMFIntegrityStatus::IndexOutsideVertexArray(*index,vertices.len() as IndexType)
                        }
                    }
...
fn verify_uvs(mesh:&TMFMesh)->TMFIntegrityStatus{
    match mesh.get_uvs(){
        None=>if mesh.get_uv_faces().is_some(){
                ...
                Some(faces)=>{
                    for index in faces{
                        if *index > uvs.len() as IndexType{
                            return TMFIntegrityStatus::IndexOutsideUVArray(*index,uvs.len() as IndexType)
                        }
                    }
                    TMFIntegrityStatus::Ok
                },
...
fn verify_normals(mesh:&TMFMesh)->TMFIntegrityStatus{
    match mesh.get_normals(){
        None=>if mesh.get_normal_faces().is_some(){
                ...
                Some(faces)=>{
                    for index in faces{
                        if *index > normals.len() as IndexType{
                            return TMFIntegrityStatus::IndexOutsideNormalArray(*index,normals.len() as IndexType)
                        }
                    }
                    TMFIntegrityStatus::Ok
                },
...
pub(crate) fn verify_tmf_mesh(mesh:&TMFMesh)->TMFIntegrityStatus{
    ...
    if vertices_status.is_err(){errors.push(vertices_status)};
    //Verify normals
    let normals_status = verify_normals(mesh);
    if normals_status.is_err(){errors.push(normals_status)};
    //Verify uvs
    let uvs_status = verify_uvs(mesh);
    if uvs_status.is_err(){errors.push(uvs_status)};
    let len = errors.len();
    match len{
